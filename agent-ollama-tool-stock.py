...
def get_stock_price(tickers: list[str]) -> str:
    ...
    for ticker in tickers:
        ...
        cache_file = f"cache/{clean_ticker}.json"
        ...
        try:
            ...
            if hist.empty:
                ...
                results.append({"ticker": clean_ticker, "error": "No data returned"})
                continue
            ...
            results.append({"ticker": clean_ticker, "error": str(e)})
...
MODEL = "qwen2.5:latest"  # or qwen2.5, mistral-nemo, etc. — must support tool use
...
while True:
    response = ollama.Client(host="http://192.168.68.101:11434").chat(
        model=MODEL,
        messages=messages,
        tools=tools,
    )
    ...
    for tool_call in assistant_message["tool_calls"]:
        ...
        if tool_name == "get_stock_price":
            print(f"\n[TOOL] get_stock_price called — fetching {len(sp500_tickers)} tickers")
